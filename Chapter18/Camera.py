...
class Camera:
    ...
    def update(self):
        ...
        if key[pygame.K_w]:
            self.transform.update_position(self.transform.get_position() + pygame.Vector3(0, 0, self.pan_speed))
        if key[pygame.K_s]:
            self.transform.update_position(self.transform.get_position() + pygame.Vector3(0, 0, -self.pan_speed))
        if key[pygame.K_a]:
            self.transform.update_position(self.transform.get_position() + pygame.Vector3(self.pan_speed, 0, 0))
        if key[pygame.K_d]:
            self.transform.update_position(self.transform.get_position() + pygame.Vector3(-self.pan_speed, 0, 0))
        if key[pygame.K_q]:
    ...
    def rotate_with_mouse(self, yaw, pitch):
        ...
        up = pygame.Vector3(self.get_VM()[0, 1], self.get_VM()[1, 1], self.get_VM()[2, 1])
